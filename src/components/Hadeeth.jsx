...
export default function Hadeeth() {
  ...
  useEffect(() => {
    fetch(
      `https://hadis-api-id.vercel.app/hadith/${
        selectedRawi.slug ? selectedRawi.slug : "abu-dawud"
      }?page=2&limit=300`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <section className="hadeeth" id="hadeeth">
      {/* <NavBar /> */}
      ...
      <div className="hadeeth-section">
          ...
          {hadeeth &&
            hadeeth.map((h, i) => {
              return (
                <SwiperSlide key={i}>
                  {hadeeth[i].arab}
                  <p className="hadeeth-sub">
                    رواه الإمام{" "}
                    {selectedRawi.slug ? selectedRawi.nameArabic : "أبو داوود"}
                  </p>
                </SwiperSlide>
              );
            })}
        ...
        >
          ...
          <Div className="salaat-place" style={{ fontFamily: "Cairo" }}>
            {selectedRawi.nameArabic ? selectedRawi.nameArabic : "أبو داوود"}
          </Div>
        </FormControl>
      </div>
    </section>
